...
export class StockTrackerComponent implements OnDestroy {
  ...
  priceHistory$ = this.stockPrice$.pipe(
    scan((history, price) => [...history.slice(-9), price], []),
    startWith([])
  );
...
export class StockTrackerComponent {
  ...
  // Resource with automatic dependency tracking
  ...
  constructor() {
    // Effect to update price history
    effect(() => {
      const price = this.currentPrice.value();
      if (price !== undefined) {
        this.priceHistory.update(history => [...history.slice(-9), price]);
      }
    });
    
    // Effect with cleanup for polling
    effect((onCleanup) => {
...
_Let's build the future of Angular together_ 🚀